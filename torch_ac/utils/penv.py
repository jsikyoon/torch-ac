...
def get_agentview(img, agent_view_size, img_encode):
    if img_encode:
...
def worker(conn, env, img_encode):
    while True:
        cmd, data = conn.recv()
        if cmd == "step":
...
class ParallelEnv(gym.Env):
    ...
    def __init__(self, envs, img_encode):
        ...
        for env in self.envs[1:]:
            ...
            p = Process(target=worker, args=(remote, env, img_encode))
            p.daemon = True
            p.start()
            remote.close()
    ...
    def env0_step(self, action):
        _, reward, done, info = self.envs[0].step(action)
        obs = self.envs[0].gen_obs()
        obs['image'] = get_agentview(obs['image'], self.envs[0].agent_view_size, self.img_encode)
